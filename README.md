...
- **Ribbon explorer**: open ribbon XML from a solution zip or local files, add command rule references, and create common enable rules. Built-in enable references include `Mscrm.SelectionCountExactlyOne`, `Mscrm.ShowOnQuickAction`, `Mscrm.ShowOnGrid`, and `Mscrm.ShowOnGridAndQuickAction`. Custom enable rules can use JavaScript, form state, field value, selection count, record privilege, entity, and command client type checks.
