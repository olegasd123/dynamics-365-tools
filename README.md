...
- `Preview Ribbon` opens a visual command-bar preview of a ribbon view or document. Custom buttons are grouped by their ribbon location and shown in sequence order with their resolved labels, and hidden out-of-the-box buttons are listed alongside them. The action is on the view and document items (inline icon and right-click menu) and on the details panel.
